...
def run_black_box_attack(oracle, universe):
    ...
    # We limit the universe scan for simulation speed on PC
...
def main():
    ...
    # N=2000, M=20000 might take 1-2 mins.
    M = 2048   # Filter size
    K = 4      # Hash functions
    ...
    # We will test different Rate Limits (Queries per "Day")
    # High Limit = Fast Attack (Data doesn't churn much)
    # Low Limit = Slow Attack (Data churns heavily before attack finishes)
    ...
    for limit in rate_limits:
        ...
        # 4. Evaluate Success
        # Success = Found elements that are CURRENTLY in the set
        # (If they were in the set at start but churned out, they are useless)
    ...
    plt.plot(rate_limits, results_recall, marker='o', label='Recall (Set Recovery)', color='blue')
    plt.plot(rate_limits, results_precision, marker='x', linestyle='--', label='Precision (Valid Info)', color='red')
